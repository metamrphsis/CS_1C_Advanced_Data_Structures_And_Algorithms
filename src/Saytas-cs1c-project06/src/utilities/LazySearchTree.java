...
public class LazySearchTree<T extends Comparable<? super T>> implements Cloneable
{
    ...
    /**
     * The method will call the recursive collectGarbage()
     * method that does the actual deletion
     * @return A boolean type of true
     */
    ...
    /**
     * The method is the recursive counterpart of the public
     * collectGarbage() method, and it takes and returns a
     * root node. This method will call the removeHard()​ utility
     * A call to this method allows the client to truly remove
     * all deleted (stale) nodes from the tree
     * @param root  The root node the method is currently on
     * @return      A root node
     */
    ...
    /**
     * The method starts searching from the root then
     * returns the true minimum or maximum node
     * @param currentNode   A LazySTNode type of Node
     *      *               to look for
     * @return              A LazySTNode type of Node to return
     */
    protected LazySTNode findMinHard(LazySTNode currentNode)
    {
        while(currentNode.lftChild != null)
        {
            currentNode = currentNode.lftChild;
        }
     ...
     * @return  A LazySTNode type of Node to return
     */
    protected LazySTNode findMaxHard(LazySTNode currentNode)
    {
        while(currentNode.rghtChild != null)
        {
            currentNode = currentNode.rghtChild;
        }
    ...
    /**
     * The method takes in two parameters; one of the
     * type LazySTNode and a generic type T then returns
     * an object of type LazySTNode for the node we just
     * removed
     * returns the true minimum or maximum node
     * @param root  The root node the method is currently on
     * @param x     A generic type of variable to remove from
     *              the tree
     * @return      An object of type LazySTNode for the node
     *              we just removed
     */
    protected LazySTNode removeHard(LazySTNode root, T x)
    {
        //LazySTNode removedNode;
        ...
        {
            root.rghtChild = removeHard(root.rghtChild, x);
        }
        else
        {
            ...
            {
                mSizeHard--;
                return root.rghtChild;
            }
            else if(root.rghtChild == null)
            {
                mSizeHard--;
            ...
            root.data = findMinHard(root.rghtChild).data;
            root.deleted = false;

            root.rghtChild = removeHard(root.rghtChild, root.data);
        }
