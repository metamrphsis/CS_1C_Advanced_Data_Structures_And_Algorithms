...
/**
 * The FHsort class contains various sorting algorithms
 * Particularly, Quick Sort, divide and conquer, algorithm
 * that recursively sorts the sub-lists of an array by choosing
 * an element called pivot from the list then arranges the element
 * to the left that are less than the pivot, to the right that are
 * greater than the pivot. This process is recursively applied to
 * the sub-list of the elements with smaller and larger values
 *
...
public class FHsort
{
    ...
    void heapSort(E[] a)
    {
        ...
        for(k = arraySize - 1; k > 0; k--)
        {
            ...
            a[0] = a[k];
            a[k] = temp;
            percolateDown( a, 0, k );  // count represents the shrinking array size
        }
    }
