...
/**
 * The PlotGraphData2D class plots the data that is
 * read via GetGraphData class
 *
 * @author Bita Mazloom, Selahittin Saytaş
 */
public class PlotGraphData2D extends Application
{
    /**
    ...
    public void start(Stage stage)
    {
        ...
        // Defining the series
        for(int i = 0; i < graphData.getNumPlots(); i++)
        {
            XYChart.Series<Number, Number> series = new XYChart.Series<>();
            series.setName("Quick Sort Data: " + i);

            // Populating the series with data
            for(int j = 0; j < graphData.getColumns(); j++)
            {
